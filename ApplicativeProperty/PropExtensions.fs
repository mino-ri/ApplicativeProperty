...
type PropExtensions =
    ...
    [<Extension>]
    static member Subscribe(getProp: IGetProp<'T>, onNext) = getProp.Subscribe(action1 onNext)
    ...
    [<Extension>]
    static member Zip(prop1, prop2, prop3, selector: Func<'T1, 'T2, 'T3, 'U>) =
        Prop.map3 (fun x y z -> selector.Invoke(x, y, z)) prop1 prop2 prop3
    ...
    [<Extension>]
    static member Apply(funcProp: IGetProp<Func<'T, 'U>>, argProp) = (funcProp, argProp) ||> Prop.map2 (fun f x -> f.Invoke(x))
