...
type internal Command(onExecute: obj -> unit, canExecute: IGetProp<bool>, context: SynchronizationContext) =
    do if isNull context then nullArg (nameof context)
    ...
    interface ICommand with
        ...
        member this.add_CanExecuteChanged(handler) =
            let disposer = canExecute.Subscribe(fun _ -> context.Post(SendOrPostCallback(fun _ ->
                handler.Invoke(this, EventArgs.Empty)), null))
